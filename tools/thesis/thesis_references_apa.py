...
SUPPLEMENTAL_CITED_IN_TEXT = [
    ...
    "Rosero Bernal, D. G. (2024). Modelo de un sistema de administracion de energia autonomo operado desde la nube para optimizar la gestion de un grupo de microredes (Tesis doctoral). Universidad Distrital Francisco Jose de Caldas.",
...
def _normalize_ref(text: str) -> str:
    ...
    ref = re.sub(r"\s*\[PV[^\]]*\]\s*", " ", ref, flags=re.IGNORECASE)
    ref = ref.replace("*", "")
    ref = re.sub(r"\s+", " ", ref)
    return ref.strip()
