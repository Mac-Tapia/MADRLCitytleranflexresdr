...
- **𝒮**: estado global = concatenación de observaciones locales (`ctde_state="concatenated_local_observations"`). La evaluación del dataset reporta un **state dim = 1856** por escenario con 17 agentes (incluye señales EV y de red); el Plan estima ~40 dims locales × 17 ≈ 680 como aproximación conceptual. `[Pendiente: documentar la composición exacta de las 1856 dimensiones.]`
- **𝒜ᵢ**: acción local del edificio i — scaling HVAC ∈ [0,1], potencia BESS ∈ [−P_max,i, +P_max,i], potencia de carga EV por cargador. Dimensionalidad 2–5 (B01 = 5).
...
| Batch | — | 1 | 512 | 512 |
| LR | 5e-4 / 5e-4 | 3e-4 / 5e-4 | 5e-4 | π 3e-4 / Q 1e-3 |
| `gamma` | 0.99 | 0.9999 | 0.99 | 0.99 |
| Otros | clip 0.2, GAE 0.95 | `axis`, bins 3 | `tau` 0.005, `target_noise` 0.2, `train_interval` 100 | `attend_heads` 4, `reward_scale` 100, `steps_per_update` 250, `num_updates` 8 |

> Nota: los valores difieren entre la corrida v4 (ajustada a 8 GB VRAM local) y la configuración canónica (objetivo Colab A100, `gamma=0.99`, `hidden_size=384`). El Capítulo 5 reporta resultados de la corrida v4 completada.
...
- **Hardware (corrida v4):** NVIDIA RTX 4060 Laptop, 8 188 MiB VRAM, driver 560.94; perfil `local4060_fast` (TF32, `cuda_memory_fraction≈0.812`, `max_split_size_mb:128`).
- **Concurrencia:** `MaxConcurrentScenarioJobs=2`; MASAC/MAAC limitados a 1 (estabilidad de memoria); `torch_threads` 8-12.
