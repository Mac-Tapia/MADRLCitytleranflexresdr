...
|-----------|------|:-----------------:|:------:|:------:|:---------:|:------------:|
| HAPPO | On-policy cooperativo | **0.70** | **0.45** | **0.35** | **0.25** | **1.00** |
| MASAC | Off-policy entropía | **0.70** | **0.45** | **0.35** | **0.25** | **1.00** |
...
## MÓDULO G — APORTES ORIGINALES AL MOTOR DE SIMULACIÓN CITYLEARN V3 PROPUESTO

This module documents four original contributions to the simulation engine implemented in the fork `github.com/Mac-Tapia/CityLearn.git`. Must be integrated into **Section 4.10.4** of the thesis plan (Chapter IV). Commit reference: `54b1938e` (CityLearn submodule), `a29cb04` (parent repo). Full academic documentation: `docs/thesis/APORTES_SIMULACION_CITYLEARN_MADRL_TESIS.md`.

### G.1 — Aporte A1: Degradación BESS con C-rate y Temperatura (Arrhenius)

**Archivo:** `CityLearn/citylearn/energy_model.py` — `Battery.degrade(temperature_celsius=25.0)`

El modelo extiende la degradación lineal original con dos factores multiplicativos:

```
ΔC = base × f_crate × f_temp

base    = capacity_loss_coefficient × capacity × |throughput| / (2 × degraded_capacity)
f_crate = (|energy_flow| / degraded_capacity)^0.55     [exponente empírico LiFePO4]
f_temp  = exp[ Ea/R × (1/T_ref − 1/T) ]               [Arrhenius, Ea=24500 J/mol]
```

A 35°C (Iquitos mediodía): f_temp ≈ 1.14. Backward-compatible: `degrade()` sin args → comportamiento original.

**Referencias clave:** Naumann et al. (2021) *J. Energy Storage* 36; Rajagopalan et al. (2024) *Applied Energy* 358.

### G.2 — Aporte A2: Corrección PV Temperatura Tropical (IEC 61215)

**Archivo:** `CityLearn/citylearn/energy_model.py` — `PV.get_generation(inverter_ac_power_per_kw, dry_bulb_temperature=None, ghi=None)`

```
T_cell = T_amb + (NOCT − 20) / 800 × GHI          [NOCT=45°C, GHI en W/m²]
P(T)   = P_STC × [1 + γ_pmax × (T_cell − 25)]     [γ=-0.0035/°C, monoSi]
```

En Iquitos (T_amb=35°C, GHI=1000 W/m²): derating ~14.4% vs. STC. Backward-compatible: sin args de temperatura → original.

**Referencias clave:** Tamoor et al. (2022) *Energy Reports* 8; IEC 61215-1:2021.

### G.3 — Aporte A3: KPI Pico de Demanda con Ventana de Facturación Configurable

**Archivo:** `CityLearn/citylearn/cost_function.py` — `CostFunction.peak(net_electricity_consumption, window=24, billing_window_steps=1)`

Añade un máximo rodante sub-horario antes de la agrupación diaria. `billing_window_steps=1` (default) reproduce el comportamiento original exactamente.

**Justificación regulatoria:** OSINERGMIN Res. 0024-2024-OS/CD — cargo por máxima demanda en ventanas de 15 minutos, tarifas MT-3/MT-4, Electro Oriente S.A. (concesionaria Iquitos).

### G.4 — Aporte A4: Clase CarbonIntensityModel para Redes Aisladas Diesel+PV

**Archivo:** `CityLearn/citylearn/energy_model.py` — nueva clase `CarbonIntensityModel`

```python
CI(t) = base_ci × (1 − pv_displacement_factor × min(GHI(t)/1000, 1))
```

Defaults: `base_ci=0.790 kgCO₂/kWh` (MINAM RAGEI Loreto 2019), `pv_displacement_factor=0.15`. Verificación: `CI(1000 W/m²) = 0.6715`, `CI(0) = 0.790`.

**Referencias clave:** MINAM (2019) RAGEI; Liu et al. (2022) *Applied Energy* 321; Tranberg et al. (2020) *Energy Strategy Reviews* 26.
...
31. Sección 4.10.4: 4 aportes originales al motor de simulación (A1–A4) con modelos matemáticos, justificación regulatoria y referencias bibliográficas 2019–2024.
